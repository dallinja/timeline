...
>(({ className, children, disablePortal, position = 'popper', ...props }, ref) => {
  const Comp = disablePortal ? React.Fragment : SelectPrimitive.Portal
  return (
    <Comp>
  ...
  return (
    <Comp {...(label ? { className: cn(fullWidth && 'w-full', className) } : {})}>
      ...
      <Select value={value} onValueChange={onValueChange} {...props}>
        <SelectTrigger
          className={cn(fullWidth ? 'w-full' : 'w-[180px]', label ? 'mt-1' : className)}
        >
          <SelectValue placeholder={placeholder || 'Select'} />
        </SelectTrigger>
        <SelectContent disablePortal={disablePortal}>{children}</SelectContent>
      </Select>
    </Comp>
  )
}
